...
class MultiStageDecoderLayer(nn.Module):
    ...
    def forward(self, vis_query, vis_query_pos, text_query_pos,
        ...
        text_info = self.word_attn(query=self.with_pos_embed(vis_query, vis_query_pos),
                                   ...
                                   value=word_feat, key_padding_mask=word_key_padding_mask)[0] #(查询文本当中有关于目标的物体的信息)
        text_query = self.norm[0](self.dropout[0](text_info))

        # Gather visual feats based on the linguistic info
        vis_info = self.img_attn(query=self.with_pos_embed(text_query, text_query_pos),  #嵌套一下来获得有关于在视觉当中的语义信息，而这个语义信息又是有关于物体的
...
class DecoderWithExtraEncoder(nn.Module):
    def __init__(self, num_queries, query_dim,
        ...
        self.extra_encoder_layers = _get_clones(extra_encoder_layer, num_extra_layers)#包含img2text_attn,img_proj,text_proj,img_textcond_attn,gamma_linear,beta_Linear等等

        args = layer.copy()
        layer_type = args.pop('type')
        decoder_layer = _MODULES[layer_type](**args) #主要包含word_attn, img_attn, ffn等模块
    ...
    def forward(self, img_feat, img_key_padding_mask=None, pos=None,
        ...
        w = h = 20
        # Encode discriminative features（总的来说经过一长串复杂式子获得表征性以及区别性很强的img_feat）
        ...
        img_feat = torch.cat([ori_img,dis_img_vd],dim=-1)
        vis_query_embed = self.vis_query_embed.weight.unsqueeze(1).repeat(1, bs, 1) #(1,B,256)
        text_query_embed = self.text_query_embed.weight.unsqueeze(1).repeat(1, bs, 1)#(1,12,256)

        # Initial target query
        vis_query = torch.zeros_like(vis_query_embed)
        img_fea = self.ffn(feature)  #(12 256)
        img_fea = img_fea.unsqueeze(0)
        gate = torch.sigmoid(self.gate_linear(torch.cat([vis_query, img_fea], dim=-1)))
        vis_query = gate * vis_query + (1-gate)*img_fea
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                intermediate.append(self.norm(vis_query))#中间过程中的vis_query也会正常记录
...
class DiscriminativeFeatEncLayer(nn.Module):
    def __init__(self, d_model, img2text_attn_args=None, img_query_with_pos=True,
        ...
        self.tf_lambda = Parameter(torch.Tensor([0.5]))
        self.tf_b = Parameter(torch.Tensor([1]))
        args = img2textcond_attn_args.copy()
        self.img2textcond_attn = MULTIHEAD_ATTNS[args.pop('type')](**args)
        self.gamma_linear = torch.nn.Linear(256, 256)
        self.beta_linear = torch.nn.Linear(256, 256)
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        text_info = self.img2text_attn(
            ...
            value=word_feat, key_padding_mask=word_key_padding_mask)[0]#在语言中，那些部分与图像相关
        ...
        gaussian_part = torch.exp( - (1 - verify_score).pow(self.tf_pow) \
                        / (2 * self.tf_sigma**2))

        laplace_part = torch.exp(-torch.abs(1 - verify_score) / self.tf_b)

        verify_score = self.tf_scale*(self.tf_lambda*gaussian_part + (1-self.tf_lambda)*laplace_part)

        # language-guided context encoder(等价于上面的text_info)
        ...
        gamma = self.gamma_linear(text_cond_info)
        beta = self.beta_linear(text_cond_info)

        # Modulate img_feat(调制一下图像特征，让图像特征里面包含一点语言信息)
        q = k = gamma * img_feat + beta
...
class MHAttentionRPE(nn.Module):
    ...
    def __init__(self, d_model, h, dropout=0.1, return_raw_attention=False,
        ...
        super().__init__()
        self.d_k = d_model // h #(每个头有多少个维度)
        self.h = h              #8个头
        self.scaling = float(self.d_k) ** -0.5
        self.return_raw_attention = return_raw_attention #Flase

        self.in_proj_weight = Parameter(torch.Tensor(3 * d_model, d_model))#(768,256)
        self.in_proj_bias = Parameter(torch.empty(3 * d_model))            #(768)
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        img_mask = ~key_padding_mask.view(bs, 20, 20)              #12 20 20
        ...
        attn_weights = attn_weights + pos_attn_weights       #经过一长串复杂式子让attn_weights更包含有空间信息之类的
