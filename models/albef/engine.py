...
class ALBEF(VLPModel):
    ...
    def load_model(self, model_id):
        ...
        if not self._models.has(model_id):
            tokenizer = BertTokenizer.from_pretrained('bert-base-uncased') #词元编码器
    ...
    def cal_score(
        ...
        for i, det in enumerate(boxes_category):
            score = gradcam[int(det[1]):int(det[3]), int(det[0]):int(det[2])]#计算特定范围内的热力图值
            area = (det[3] - det[1]) * (det[2] - det[0])#计算BBOX面积大小
            score = score.sum()
            score /= area ** 0.5
    ...
    def visualize_groundvlp(
        ...
        image_pil = Image.open(image_path).convert('RGB')
        image = transform(image_pil).unsqueeze(0).to(self.device)    #(1,3,256,256)

        model.text_encoder.encoder.layer[block_num].crossattention.self.save_attention = True
        
        text_input = tokenizer(self.templates.format(query), return_tensors="pt").to(self.device) #文本编码
        ...
        # Find agent(开始进入下游网络时刻)
        agent = self.find_agent(query)#(找到语句当中的主体词)
        mapped_coco_label = self.map_to_coco_label(agent)#将主体词映射到coco数据集当中，找到对应的类别
        # Obtain the boxes_category
        boxes_category, od_scores = self.get_bbox_for_rec(image_path, mapped_coco_label, threshold=0.15, general=True)#输入原始图片，要找的对应的类别，以及相应的阈值

        # Visual-Word Attention Aggregation
        start_idx = len(tokenizer(self.templates.format(""), return_tensors='pt').input_ids[0]) - 1 #根据模板找到起始位置
        main_words_ids, id_map = self.find_main_words(query, start_idx, tokenizer)#找到重要的词以及对应的编号
        all_focus_ids = []
        ...
        gradcam = gradcam.view(1, 1, num_patch, num_patch)
        gradcam = F.interpolate(gradcam, size=(image_pil.size[1], image_pil.size[0]),
                                mode='bicubic', align_corners=False).squeeze()#将热力图图像大小调整为原图像大小
    ...
    def show_groundvlp(
        ...
        np.clip(gradcam_image, 0., 1., out=gradcam_image)#计算热力图
