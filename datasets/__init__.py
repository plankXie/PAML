...
train_transforms = [
    dict(
        type='RandomSelect',
        transforms1=dict(type='RandomResize', sizes=[256]),
        transforms2=dict(
            type='Compose',
            transforms=[
                ...
                dict(type='RandomResize', sizes=[256])
            ],
        ),
        p=0.5
    ),
    ...
    dict(type='NormalizeAndPad', size=256, aug_translate=True)
]

test_transforms = [
    dict(type='RandomResize', sizes=[256], record_resize_info=True),
    dict(type='ToTensor', keys=[]),
    dict(type='NormalizeAndPad', size=256, center_place=True)
]